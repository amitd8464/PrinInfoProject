...
<head>
    ...
    <style>
        ...
        /* Highlight rows on hover, unless already selected */
        tr.selectable:hover:not(.selected) {
            background-color: #e0f0ff; /* light hover color */
        }

        /* Selected row (even on hover) */
...
<body>
    ...
    <div class="page-container">
        <div class="container">
                    ...
                    <script>
                        ...
                        for (var i = 0; i < tripRadios.length; i++) {
                            tripRadios[i].addEventListener('change', function() {
                                const isRoundTrip = (this.value === 'roundtrip');
                                returnField.style.display = isRoundTrip ? 'block' : 'none';

                                // Only clear return selection if changing back to one-way
                                if (!isRoundTrip) {
                                    document.getElementById('selected_return_index').value = '';
                                    document.querySelectorAll('tr.selectable.return').forEach(r => r.classList.remove('selected'));
                                }
            ...
            <!-- DEPARTURES TABLE -->
            <form action="bookFlightTicket.jsp" method="POST" onsubmit="return validateSelection();">
                <input type="hidden" name="selected_departure_index" id="selected_departure_index" />
                <input type="hidden" name="selected_return_index" id="selected_return_index" />
                    ...
                    <table class="flights-table">
                        ...
                        <% for (Map<String,Object> f : departureFlights) {
                               ...
                               String dur = (d == null)
                                            ...
                                            : (d/60 + " h " + d%60 + " min");
                        %>
                        <tr class="selectable departure" onclick="selectFlight(this, 'departure')" data-flight-index="<%= departureFlights.indexOf(f) %>">
                                <td>$<%=f.get("price")%></td>
                    ...
                    <table class="flights-table">
                        ...
                        <% for (Map<String,Object> f : returnFlights) {
                               ...
                               String dur = (d == null)
                                            ...
                                            : (d/60 + " h " + d%60 + " min");
                        %>
                        <tr class="selectable return" onclick="selectFlight(this, 'return')" data-flight-index="<%= returnFlights.indexOf(f) %>">
                                <td>$<%=f.get("price")%></td>
                                <td><%=f.get("flight_number")%></td>
                                <td><%=f.get("airline_id")%></td>
                                <td><%=f.get("dep_airport")%></td>
                ...
                <div style="margin-top: 20px; text-align: center;">
                    <button type="submit" id="continueButton" style="display: none;">Continue to Booking</button>
                </div>
            </form>
            ...
            <script>
                ...
                function validateSelection() {
                    const dep = document.getElementById('selected_departure_index').value;
                    const ret = document.getElementById('selected_return_index').value;
                    const isRoundTrip = document.querySelector('input[name="trip_type"][value="roundtrip"]')?.checked;
                ...
                function selectFlight(row, type) {
                    // Deselect previous selection of the same type
                    document.querySelectorAll(`tr.selectable.${type}`).forEach(r => {
                    ...
                    // Save selected index to hidden input
                    const index = row.getAttribute('data-flight-index');
                    document.getElementById(`selected_${type}_index`).value = index;
                ...
                function updateContinueButtonVisibility() {
                    ...
                    const show = isRoundTrip ? (dep && ret) : dep;
                    document.getElementById('continueButton').style.display = show ? 'inline-block' : 'none';
                }
        
        
        
                
                // On page load, categorize rows for easier selection
                document.querySelectorAll('h3.resultTitle').forEach((title) => {
                    const table = title.nextElementSibling.nextElementSibling;
                    const rows = table.querySelectorAll('tbody tr');
                    const type = title.textContent.includes('Departure') ? 'departure' : 'return';
                    rows.forEach(row => row.classList.add(type));
                });
        
                // Detect manual page reloads
                const navEntry = performance.getEntriesByType("navigation")[0];
                if (navEntry && navEntry.type === "reload") {
                    alert("Selected flights and search criteria will be cleared!");
                }
            </script>       
    </div>
